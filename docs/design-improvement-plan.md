...
| 2026-08-31 | Uploaded text is decoded by detecting its encoding instead of being forced through UTF-8 (#2789). `errors="replace"` turned every non-ASCII sequence in a Shift_JIS, EUC-JP or GB18030 file into U+FFFD before it reached the database — a Japanese upload arrived as replacement characters and could not be repaired, because the original bytes were gone. Candidates are now tried strictly in order and the first clean decode wins; nothing is ever replaced, and a file matching nothing is rejected with a 422 rather than stored broken. Two traps handled: ISO-2022-JP is 7-bit so it decodes "successfully" as UTF-8 into escape gibberish and has to be spotted by its escape sequence, and NUL bytes are valid UTF-8 so binary files pass a strict decode — every candidate is gated on the result reading as prose rather than control characters | services/book_parser.py, routers/uploads.py | #2789 |
| 2026-08-31 | An uploaded book can be discarded from the bookshelf. `DELETE /upload/{id}` and `deleteUploadedBook()` both existed and worked, but nothing in the UI ever called them — and the admin books list excludes uploads by design (`source != 'upload'`, their titles are private to the owner), so its Delete button could never reach one. An upload was therefore impossible to remove from any screen once created, which is how a broken Japanese upload (#2789) became unremovable. A Discard button now sits on the "In progress" card beside Continue audit, behind an inline confirm rather than a blocking dialog (#1897), naming what is destroyed: the chapters and any notes on them | app/(shell)/bookshelf/page.tsx | #2789 |
