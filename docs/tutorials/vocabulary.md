...
If the word you clicked is an inflected form, the panel resolves it to its base form — looking up *gegangen* shows the meaning of *gehen* ("to go, to walk"), with a note that it is the past participle. The Save button names the word it will file, so you always know what lands in your list.
...
Words are always filed under their base form, so meeting *acknowledge*, *acknowledged* and *acknowledging* across three chapters gives you one entry with three occurrences rather than three separate entries. When the base form can't be determined — an unusual proper noun, or the dictionary being unreachable — the word is saved exactly as it appeared in the text.
