 ...
 * The reader toggle must be accessible on mobile, not only `hidden lg:flex`.
 * The toggle's remit changed with #2752: it now shows/hides COMMUNITY notes
 * (own marks always render) — the accessibility contract is unchanged.
 */
...
describe("Mobile annotation toggle accessible below lg breakpoint (closes #2568)", () => {
  it("desktop annotation toggle (hidden lg:flex) still exists alongside new mobile toggle", () => {
    ...
    // Mobile toggle: the same showShares control inside the mobile notes panel
    ...
    expect(mobilePanel).toContain("showShares");
  });
  ...
  it("mobile notes panel contains an aria-pressed toggle for showAnnotations", () => {
    ...
    expect(panel).toContain("aria-pressed={showShares}");
  });

  it("mobile annotation toggle has an aria-label", () => {
    // Find all aria-pressed={showShares} occurrences and verify each has aria-label nearby
    let searchFrom = 0;
    let found = 0;
    while (true) {
      const idx = src.indexOf("aria-pressed={showShares}", searchFrom);
      if (idx === -1) break;
      found++;
      searchFrom = idx + 1;
    }
