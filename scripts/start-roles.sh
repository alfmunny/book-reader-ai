...
SESSION STARTUP — before invoking /loop below, perform the full CLAUDE.md §Session startup sequence (steps 1–6) for the Dev role. CLAUDE.md is at ${REPO}/CLAUDE.md and is auto-loaded as the project file. Read every memory file listed in ${MEMORY_PATH}. Verify your worktree exists at ${REPO_DEV} (run: git -C ${REPO} worktree list). Walk through every leftover PR you authored (gh pr list --author @me --state open --json number,title,mergeStateStatus,headRefName): BEHIND → fetch+rebase+force-push; failing CI → investigate, fix, force-push; BLOCKED waiting on PM → comment and move on; otherwise leave for auto-merge. Only after every leftover PR has been accounted for, schedule the recurring Dev work loop by invoking:
...
SESSION STARTUP — before invoking /loop below, perform the full CLAUDE.md §Session startup sequence (steps 1–6) for the UI/UX Dev role. CLAUDE.md is at ${REPO}/CLAUDE.md and is auto-loaded as the project file. Read every memory file listed in ${MEMORY_PATH}. Verify your worktree exists at ${REPO_UIUX} (run: git -C ${REPO} worktree list). Walk through every leftover PR you authored (gh pr list --author @me --state open --json number,title,mergeStateStatus,headRefName): BEHIND → fetch+rebase+force-push; failing CI → investigate, fix, force-push; BLOCKED waiting on PM → comment and move on; otherwise leave for auto-merge. Only after every leftover PR has been accounted for, schedule the recurring UI/UX work loop by invoking:
...
SESSION STARTUP — before invoking /loop below, perform the full CLAUDE.md §Session startup sequence (steps 1–6) for the Architect role. CLAUDE.md is at ${REPO}/CLAUDE.md and is auto-loaded as the project file. Read every memory file listed in ${MEMORY_PATH}. Verify your worktree exists at ${REPO_ARCH} (run: git -C ${REPO} worktree list). Walk through every leftover PR you authored (gh pr list --author @me --state open --json number,title,mergeStateStatus,headRefName): BEHIND → fetch+rebase+force-push; failing CI → investigate, fix, force-push; BLOCKED waiting on PM → comment and move on; otherwise leave for auto-merge. Only after every leftover PR has been accounted for, schedule the recurring Architect work loop by invoking:
